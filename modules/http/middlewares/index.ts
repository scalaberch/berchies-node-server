 ...
 * @param next
 */
export const applyRequestFunctions = (req: HttpRequest, res: HttpResponse, next: HttpNext) => {
  // Get basic jwt data
  ...
  req.getCookie = (key: string) => getCookie(req, key);
 ...
 * @param next
 */
export const applyResponseFunctions = (req: HttpRequest, res: HttpResponse, next: HttpNext) => {
  res.outputSuccess = (payload: any, message?: string) =>
    outputSuccess(res, message, payload, req.access);
  res.outputJson = (payload: any, code = 200) => output(res, payload, code);
  res.outputError = (message = 'Error.', payload?: any, code = 200) =>
    outputError(res, message, payload, code);
  res.outputAsCSV = (dataset: any, fileName?: string) => outputAsCsv(res, dataset, fileName);
  res.outputCreated = (payload: any, message?: string) =>
    outputCreated(res, message, payload, req.access);
  ...
  res.setCookie = (
    ...
    sameSite: HttpCookieSameSite = 'none',
    secure = true,
    httpOnly = true,
    options: CookieOptions = {},
  ) => setCookie(res, name, value, path, maxAge, sameSite, secure, httpOnly, options);
